...
class Arm
{
    ...
    {
        //x^2 + y^2 - l1^2 - l2^2
        const term1 = this.x * this.x + this.y * this.y - this.length1 * this.length1 - this.length2 * this.length2;
        //2*l1*l2
        ...
        this.toTheta1 = Math.atan(this.y / this.x) - Math.atan(a / b);

        //fix angles if the arm is below to the x-axis
        const angle= this.length1 * Math.sin(this.toTheta1);
        if (angle < 0)
        {
            const h= Math.sqrt((this.x * this.x) + (this.y * this.y));
            const thetaH= Math.acos(this.x/h);
    ...
    drawArm(canvas)
    {
        ...
        this.drawDot(ctx, 0, canvas.height);
        this.drawSegment(ctx, 0, canvas.height, this.length1, this.theta1);
        this.drawDot(ctx, (this.length1*Math.cos(this.theta1)), canvas.height-(this.length1*Math.sin(this.theta1)));
        this.drawSegment(ctx, (this.length1*Math.cos(this.theta1)), canvas.height-(this.length1*Math.sin(this.theta1)), this.length2, this.theta2 + this.theta1);
